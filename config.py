...
class RealWorldReference:
    ...
    ИСТОЧНИКИ
      ...
      Стоимость накопителя — мировой рынок BESS 2025: обследование BNEF даёт
        средневзвешенную цену системы «под ключ» около 117 $/кВт·ч (Европа
        ~177, США ~219); диапазон 200–400 $/кВт·ч соответствовал примерно
        2019–2021 годам. Для РФ принято 16 000 руб/кВт·ч — выше европейской
        цены с учётом импорта и монтажа.
    ...
    # Порог поднят с 0.50: прежнее значение проходило даже для ряда без
    # выраженного суточного цикла и не могло провалиться. Измеренное значение
    # генератора после привязки промышленных смен к часам суток — около 0.72.
    # Реальный агрегат UCI из 370 клиентов даёт 0.99, отдельные клиенты
    # 0.75-0.98; генератор описывает город меньшего размера и потому шумнее.
    ACF24_MIN: float = 0.65
    # Полдень обязан быть выше ночного минимума, а утренний подъём — заметен.
    # Без этих проверок генератор выдавал профиль, где полдень ниже трёх часов
    # ночи, и ни один показатель этого не улавливал.
    NOON_OVER_NIGHT_MIN: float = 1.15
    MORNING_OVER_NIGHT_MIN: float = 1.10
    WINTER_SUMMER_MONTH_RATIO_MIN: float = 1.08
    ...
    # ── Накопитель ───────────────────────────────────────────────────────────
    # ≈195 $/кВт·ч под ключ. Прежние 25 000 (≈300 $) отражали цены начала
    # 2020-х и завышали окупаемость вдвое: именно на них держался вывод, что
    # сглаживание пика не оправдывает установку.
    BATTERY_CAPEX_RUB_PER_KWH: float = 16_000.0
...
class Config:
    ...
    # Рабочее окно заряда согласовано с глубиной разряда, по которой считается
    # ресурс (BATTERY_REF_DOD = 0.80). Прежние 0.25–0.75 давали 50% полезной
    # ёмкости, то есть фактически двухчасовой накопитель при объявленном
    # четырёхчасовом, а удельная деградация считалась для режима, который
    # контроллер никогда не выполнял.
