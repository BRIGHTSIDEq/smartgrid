# -*- coding: utf-8 -*-
"""config.py — Smart Grid v13.

ИЗМЕНЕНИЯ v13:
  set_optimal_mode():
    HISTORY_LENGTH: 48 → 168  (NN теперь видят полный недельный паттерн ACF168=0.789)
    PATIENCE:       20 → 30   (больше времени для сходимости на длинных окнах)
    LR_PATIENCE:    8  → 10
    MIN_DELTA:      0.0 → 1e-5 (EarlyStopping не реагирует на шум < 1e-5)
    BATCH_SIZE:     32 → 16   (больше шагов/эпоха при большем окне)
    LSTM_TCN_FILTERS: 48 → 64
"""
...
class Config:
    ...
    def set_fast_mode(cls):
        ...
        cls.VANILLA_USE_SEASONAL_RESIDUAL = False; cls.VANILLA_SEASONAL_BLEND_INIT = 0.35
        ...
        logging.getLogger("smart_grid").info(
            "Fast mode v13: DAYS=%d EPOCHS=%d HISTORY=%d N_FEATURES=%d | "
            "LSTM BiLSTM=%d TCN=%d | VanTr LR=%.0e | EV=%.0f%%",
            cls.DAYS, cls.EPOCHS, cls.HISTORY_LENGTH, cls.N_FEATURES,
    ...
    def set_optimal_mode(cls):
        """
        v13 изменения vs v12:
          HISTORY_LENGTH: 48 → 168   NN теперь видят полный недельный паттерн (ACF168=0.789)
          PATIENCE:       20 → 30    больше времени для сходимости на длинных окнах
          LR_PATIENCE:    8  → 10
          MIN_DELTA:      0.0 → 1e-5  EarlyStopping не реагирует на шум < 1e-5
          BATCH_SIZE:     32 → 16    больше шагов/эпоха (окно выросло в 3.5x)
          LSTM_TCN_FILTERS: 48 → 64  компенсация более длинной последовательности
        """
        cls.DAYS = 730; cls.HOUSEHOLDS = 2500; cls.EPOCHS = 240
        cls.PATIENCE = 30; cls.LR_PATIENCE = 10; cls.MIN_DELTA = 1e-5
        cls.HISTORY_LENGTH = 168; cls.STORAGE_HORIZON = 720; cls.N_FEATURES = 26
        ...
        cls.DROPOUT_RATE = 0.15; cls.LSTM_LEARNING_RATE = 1.0e-4; cls.LSTM_USE_COSINE_DECAY = False
        ...
        cls.TRANSFORMER_DROPOUT = 0.12; cls.TRANSFORMER_LEARNING_RATE = 1.0e-4
        cls.VANILLA_TRANSFORMER_LR = 8e-5; cls.TRANSFORMER_STOCHASTIC_DEPTH = 0.08
        cls.PATCHTST_USE_REVIN = True
        cls.VANILLA_USE_SEASONAL_RESIDUAL = False; cls.VANILLA_SEASONAL_BLEND_INIT = 0.40
        ...
        logging.getLogger("smart_grid").info(
            "Optimal mode v13: DAYS=%d HISTORY=%d EPOCHS=%d PATIENCE=%d BATCH=%d | "
    ...
    def set_full_mode(cls):
        ...
        cls.PATIENCE = 35; cls.LR_PATIENCE = 12; cls.MIN_DELTA = 1e-5
        ...
        logging.getLogger("smart_grid").info(
            "Full mode v13: DAYS=%d HH=%d EPOCHS=%d HIST=%d PATIENCE=%d | "
            ...
            cls.LSTM_LEARNING_RATE, cls.DROPOUT_RATE,
            cls.TRANSFORMER_D_MODEL, cls.TRANSFORMER_N_HEADS, cls.TRANSFORMER_N_LAYERS,
            cls.TRANSFORMER_DFF, cls.TRANSFORMER_LEARNING_RATE, cls.VANILLA_TRANSFORMER_LR,
