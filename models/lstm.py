...
class ConsumptionRevIN(tf.keras.layers.Layer):
    """
    Нормировка канала потребления по статистикам самого окна (RevIN).

    Канал 0 приводится к нулевому среднему и единичному СКО в пределах каждого
    входного окна, ковариаты передаются без изменений. Это снимает с сети
    задачу подстройки под медленный дрейф уровня нагрузки (годовой тренд,
    сезонный сдвиг) и оставляет ей форму профиля.

    Реализовано отдельным слоем, а не через Lambda: слой с Python-лямбдой
    внутри не поддаётся безопасной десериализации, из-за чего сохранённую
    модель невозможно загрузить обратно — инференс-контур оказывался нерабочим.
    """
    ...
    def call(self, inputs):
        cons = inputs[:, :, :1]
        covariates = inputs[:, :, 1:]
        mean = tf.reduce_mean(cons, axis=1, keepdims=True)
        std = tf.math.reduce_std(cons, axis=1, keepdims=True) + self.eps
        return tf.concat([(cons - mean) / std, covariates], axis=-1)
...
def build_lstm_model(
    ...
    x = ConsumptionRevIN(name="revin_consumption")(inp)
    ...
    model = tf.keras.Model(inputs=inp, outputs=final_out, name="TCN_BiLSTM_Attention")
    model.compile(
    ...
    logger.info(
        "TCN-BiLSTM-Attention | %d params | input=(%d,%d) | "
